...
public class JMSConnectionFactoryHandler implements IJMSConnectionFactoryProvider {
     ...
     */
    private void setConnectionFactoryProperties() {
        for (PropertyDescriptor descriptor : propertyDescriptors) {
            String propertyName = descriptor.getName();
            if (descriptor.isDynamic()) {
                this.setProperty(propertyName, context.getProperty(descriptor).evaluateAttributeExpressions().getValue());
            } else {
                if (descriptor.equals(JMS_BROKER_URI)) {
                    String brokerValue = context.getProperty(descriptor).evaluateAttributeExpressions().getValue();
                    if (context.getProperty(JMS_CONNECTION_FACTORY_IMPL).evaluateAttributeExpressions().getValue().startsWith("org.apache.activemq")) {
